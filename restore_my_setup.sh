...
if [ ! -f /etc/arch-release ]; then
    ...
    read -p " :: Press Enter to continue anyway, or Ctrl+C to abort..." </dev/tty 2>/dev/null ||     read -p " :: Press Enter to continue anyway, or Ctrl+C to abort..."
fi
...
if [ -n "$XDG_CURRENT_DESKTOP" ] && [ "$XDG_CURRENT_DESKTOP" != "Hyprland" ]; then
    ...
    read -p "Wanna start the setup? (This setup will change your desktop environment to Hyprland + HyDE) (y/n): " confirm_setup </dev/tty 2>/dev/null ||     read -p "Wanna start the setup? (This setup will change your desktop environment to Hyprland + HyDE) (y/n): " confirm_setup
...
read -p "Do you want to run the full system restoration setup? (y/n) [y]: " main_confirm </dev/tty 2>/dev/null || read -p "Do you want to run the full system restoration setup? (y/n) [y]: " main_confirm
...
read -p "Do you want to install/upgrade required system packages? (y/n) [y]: " opt_install_pkgs </dev/tty 2>/dev/null || read -p "Do you want to install/upgrade required system packages? (y/n) [y]: " opt_install_pkgs
opt_install_pkgs="${opt_install_pkgs:-y}"

# 3. HyDE Desktop Environment framework deployment preference
read -p "Do you want to deploy/upgrade the HyDE Desktop Environment framework? (y/n) [y]: " opt_deploy_hyde </dev/tty 2>/dev/null || read -p "Do you want to deploy/upgrade the HyDE Desktop Environment framework? (y/n) [y]: " opt_deploy_hyde
opt_deploy_hyde="${opt_deploy_hyde:-y}"

# 4. Desktop Custom Dotfiles deployment preference
read -p "Do you want to write/overwrite customized dotfiles (Hyprland, Waybar, Zsh, Kitty, Cava, VS Code, Zen)? (y/n) [y]: " opt_deploy_dots </dev/tty 2>/dev/null || read -p "Do you want to write/overwrite customized dotfiles (Hyprland, Waybar, Zsh, Kitty, Cava, VS Code, Zen)? (y/n) [y]: " opt_deploy_dots
opt_deploy_dots="${opt_deploy_dots:-y}"

# 5. Helper scripts (hotspot, nightlight, etc.) installation preference
read -p "Do you want to write/overwrite custom helper scripts (starthotspot.sh, nightlight-gui, display settings)? (y/n) [y]: " opt_helper_scripts </dev/tty 2>/dev/null || read -p "Do you want to write/overwrite custom helper scripts (starthotspot.sh, nightlight-gui, display settings)? (y/n) [y]: " opt_helper_scripts
...
read -p "Select choice [1-5] (Default 1): " gpu_choice </dev/tty 2>/dev/null || read -p "Select choice [1-5] (Default 1): " gpu_choice
...
echo -e "${CYAN}Downloading standard Arabic symbols file and configuring local XKB override...${NC}"
mkdir -p "$HOME/.config/xkb/symbols"
curl -sSL "https://gitlab.freedesktop.org/xkeyboard-config/xkeyboard-config/-/raw/master/symbols/ara" -o "$HOME/.config/xkb/symbols/ara"
cat << 'XKBEOF' >> "$HOME/.config/xkb/symbols/ara"
...
# Install EnvyControl (GPU switcher)
echo -e "\n${BLUE}${BOLD}Installing EnvyControl (GPU switching utility)...${NC}"
mkdir -p "$HOME/.local/bin"
curl -sSL "https://raw.githubusercontent.com/bayasdev/envycontrol/main/envycontrol.py" -o "$HOME/.local/bin/envycontrol"
chmod +x "$HOME/.local/bin/envycontrol"
echo -e "${GREEN}[OK] EnvyControl installed successfully to ~/.local/bin/envycontrol${NC}"

# Install Desktop Settings GUI
echo -e "\n${BLUE}${BOLD}Installing Desktop Settings GUI...${NC}"
mkdir -p "$HOME/.local/bin"
if [ -f "$SCRIPT_DIR/settings_gui.py" ]; then
    cp "$SCRIPT_DIR/settings_gui.py" "$HOME/.local/bin/settings_gui.py"
else
    echo -e "${YELLOW}GUI app not found locally. Downloading from GitHub...${NC}"
    curl -sSL "https://raw.githubusercontent.com/omarahmed321/cachy-new-config/main/settings_gui.py" -o "$HOME/.local/bin/settings_gui.py"
fi
chmod +x "$HOME/.local/bin/settings_gui.py"
echo -e "${GREEN}[OK] Settings GUI installed successfully to ~/.local/bin/settings_gui.py${NC}"
fi
...
read -p "Choose option [1-2] (Default 1): " choice </dev/tty 2>/dev/null || read -p "Choose option [1-2] (Default 1): " choice
choice="${choice:-1}"

if [ "$choice" -eq 2 ]; then
    read -p "Enter new SSID (Press Enter for default '$SSID'): " new_ssid </dev/tty 2>/dev/null ||     read -p "Enter new SSID (Press Enter for default '$SSID'): " new_ssid
    SSID="${new_ssid:-$SSID}"
    while true; do
        read -p "Enter new Password (min 8 chars, Press Enter for default): " new_pass </dev/tty 2>/dev/null ||         read -p "Enter new Password (min 8 chars, Press Enter for default): " new_pass
    ...
    read -p "Save this configuration as default? (y/n) [y]: " save_choice </dev/tty 2>/dev/null ||     read -p "Save this configuration as default? (y/n) [y]: " save_choice
...
#   Repo:    https://github.com/omarahmed321/cachy-new-config
#
#   Two modes:
...
echo -e "${CYAN}Writing VS Code password-store argv.json configs...${NC}"
mkdir -p "$HOME/.vscode"
...
cat << 'EOF' > "$HOME/.vscode-oss/argv.json"
{
    "password-store": "gnome-libsecret"
}
EOF
...
exec-once = gnome-keyring-daemon --start --components=secrets,pkcs11,keyring
env = PASSWORD_STORE,gnome-libsecret

# Launch Settings GUI shortcut
bind = $mainMod+Alt, G, exec, python3 $HOME/.local/bin/settings_gui.py # launch Desktop Settings GUI
...
if [ -f "$HOME/.config/hypr/monitors.conf" ]; then
    echo -e "${GREEN}[INFO] Existing ~/.config/hypr/monitors.conf found. Preserving your custom display settings (resolution, refresh, scale, offsets).${NC}"
else
...
if [ "$IS_LAPTOP" = "true" ] || [ "$HAS_TOUCHPAD" = "true" ]; then
    ...
    read -p "Touch pad like windows? (y/n): " tp_windows_choice </dev/tty 2>/dev/null ||     read -p "Touch pad like windows? (y/n): " tp_windows_choice
...
1|30|top|hyprland/workspaces|clock custom/prayer|network bluetooth battery pulseaudio custom/separator memory custom/separator cpu
EOF
...
if [ -d "${SRC_DIRS[0]}" ]; then
    ...
    if [ -f "$SCRIPT_DIR/patch_driver.py" ]; then
        ...
        curl -sSL -o /tmp/patch_driver.py https://raw.githubusercontent.com/omarahmed321/cachy-new-config/main/patch_driver.py || wget -q -O /tmp/patch_driver.py https://raw.githubusercontent.com/omarahmed321/cachy-new-config/main/patch_driver.py
...
ZENUSERJS
        ...
        # Write zen-themes.css
        cat << 'ZENTHEMESCSS' > "$profile/chrome/zen-themes.css"
/* Zen Mods - Generated by ZenMods.
* FILE GENERATED AT: Tuesday, July 7, 2026 at 2:02:18 AM Eastern European Summer Time
...
/* Name: Hide Toolbar */
/* Description: Allows you to hide the toolbar/navbar, including in *single toolbar mode*. */
/* Author: @devdinc */
...
ZENTHEMESCSS
        ...
        # Write e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/chrome.css
        cat << 'ZENMODCSS' > "$profile/chrome/zen-themes/e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/chrome.css"
#zen-context-menu-compact-mode-hide-toolbar[disabled="true"] {
  display: block; /* or flex-direction: column if using flex */
...
ZENMODCSS
        ...
        # Write e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/preferences.json
        cat << 'ZENMODPREFS' > "$profile/chrome/zen-themes/e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/preferences.json"
[
    {
        "property": "zen.view.compact.hide-toolbar",
        "label": "Enable the mod",
        "type": "checkbox",
        "defaultValue": true
    },
    {
        "property": "mod.devdinc.hide-navbar.animation.enabled",
        "label": "Enable animations",
        "type": "checkbox",
        "defaultValue": true
    },
    {
        "property": "mod.devdinc.hide-navbar.element-seperation",
        "label": "Change the top border at the top of the page",
        "type": "string",
        "defaultValue": "var(--zen-element-separation)"
    }
]
ZENMODPREFS

        # Write e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/readme.md
        cat << 'ZENMODREADME' > "$profile/chrome/zen-themes/e34745fd-2b7f-4c16-b03a-6e29e5c3f20a/readme.md"
# Hide Toolbar
...
if [ -f "$SCRIPT_DIR/wallpapers/background_for_me.jpg" ]; then
    ...
    echo -e "${YELLOW}Wallpaper not found locally. Downloading background_for_me.jpg from GitHub...${NC}"
    curl -sSL "https://raw.githubusercontent.com/omarahmed321/cachy-new-config/main/wallpapers/background_for_me.jpg" -o "$HOME/.config/hyde/themes/Gruvbox Retro/wallpapers/background_for_me.jpg"
fi
...
read -p "Do you want to permanently disable account lockouts on this system? (y/n) [n]: " faillock_choice </dev/tty 2>/dev/null || read -p "Do you want to permanently disable account lockouts on this system? (y/n) [n]: " faillock_choice
...
read -p "Would you like to reboot the system now to apply all changes? (y/n) [y]: " reboot_confirm </dev/tty 2>/dev/null || read -p "Would you like to reboot the system now to apply all changes? (y/n) [y]: " reboot_confirm
